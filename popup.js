document.addEventListener('DOMContentLoaded', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        ...
        // Function to save table results to CSV
        function saveResultsToCsv() {
            var csv = tableToCsv(resultsTable);
            var filename = 'Appliance_Repair.csv';
            downloadCsv(csv, filename);
            alert("All results recorded successfully!");
        }

        // Function to start the countdown for autosave
        function startAutosaveCountdown() {
            var countdown = 10;
            var countdownElement = document.createElement('div');
            countdownElement.textContent = `Results will autosave to file in... ${countdown}`;
            countdownElement.style.marginTop = '10px';
            resultsTable.parentNode.insertBefore(countdownElement, resultsTable.nextSibling);
            ...
            var countdownInterval = setInterval(() => {
                ...
                if (countdown <= 0) {
                    ...
                    saveResultsToCsv();
                    startNextQueryCountdown();
                }
            }, 1000);
        }

        // Function to start the countdown for the next search query
        function startNextQueryCountdown() {
            var countdown = 5;
            var countdownElement = document.createElement('div');
            countdownElement.textContent = `Next search query in queue begins in... ${countdown}`;
            resultsTable.parentNode.insertBefore(countdownElement, resultsTable.nextSibling);
        ...
        // Enable autosave countdown when download button is enabled
...
function nextCityQuery() {
    // Fetch the query from the "query" tab
    const query = getQueryFromTab("query");

    // Fetch the cities and states from the "cities" tab
    const citiesStates = getCitiesStatesFromTab("cities");

    // Iterate over each state and city pair
    for (let i = 0; i < citiesStates.length; i++) {
        const state = citiesStates[i].state;
        const city = citiesStates[i].city;

        // Construct the city+state search term
        const cityStateSearchTerm = `${city} ${state}`;

        // Simulate entering the city+state in the search box
        enterSearchTerm(cityStateSearchTerm);

        // Wait for the map to orient to the city+state
        setTimeout(() => {
            // Construct the full search query
            const fullSearchQuery = `${query} ${city} ${state}`;

            // Simulate entering the full search query in the search box
            enterSearchTerm(fullSearchQuery);

            // Wait for the search results to load
            setTimeout(() => {
                // Proceed with the logic from loadresults.js
                loadResults();
            }, 5000); // Wait 5 seconds
        }, 5000); // Wait 5 seconds
    }
...
function getQueryFromTab(tabName) {
    // Logic to fetch the query from the specified tab
    // Placeholder implementation
    return "example query";
}

function getCitiesStatesFromTab(tabName) {
    // Logic to fetch the cities and states from the specified tab
    // Placeholder implementation
    return [
        { state: "State1", city: "City1" },
        { state: "State2", city: "City2" }
        // Add more city-state pairs as needed
    ];
}

function enterSearchTerm(searchTerm) {
    // Logic to simulate entering a search term in the search box
...
function loadResults() {
    // Logic to load results, similar to loadresults.js
    // Placeholder implementation
